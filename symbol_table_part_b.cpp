...
// Intitialize tree
static shared_ptr<Node> tree = shared_ptr<Node> (new Node());
...
void c_file_processing (char** argv)
{
    ...
    // Puts the identifiers in the tree
    for (identifier my_identifier : identifiers)
    {
        tree->update(my_identifier);
    }
...
void identifier_maker (const string line, const int line_counter)
{
    ...
    while (stream >> token)
    {
        ...
        // Checks if the token is the type of the variable/function/array
